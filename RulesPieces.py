...
def rook(source, destination, board):
	...
	if abs(deltaX) > 0 and abs(deltaY) == 0:
		#Leftie!
		if(deltaX < 0):
			deltaPositions = range(source[1], destination[1])
		#Rightie!
		else:
			deltaPositions = range(destination[1], source[1])
		for index in deltaPositions:
			if board[source[0]][index].color != 'EMPTY' and board[index][source[1]] is not sourceLocation:
				return False
		return True
	...
	#Going high!
		...
		#Downie!
			...
			deltaPositions = range(destination[0], source[0])
		
		for index in deltaPositions:
			if board[index][source[1]].color != 'EMPTY' and board[index][source[1]] is not sourceLocation:
				return False
		return True
